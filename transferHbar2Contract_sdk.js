...
async function main() {
	...
	const hbarAmount = 10;
	const transferRx = await hbarTransferFcn(aliceId, contractId.toString(), hbarAmount);
...
async function accountCreatorFcn(pvKey, iBal) {
	const response = await new AccountCreateTransaction().setInitialBalance(new Hbar(iBal)).setKey(pvKey.publicKey).execute(client);
	const receipt = await response.getReceipt(client);
	return [receipt.status, receipt.accountId];
}

async function contractCreatorFcn(contractBytecode) {
	const contractDeployTx = await new ContractCreateFlow().setBytecode(contractBytecode).setGas(100000).execute(client);
...
async function hbarTransferFcn(sender, receiver, amount) {
	const transferTx = new TransferTransaction().addHbarTransfer(sender, -amount).addHbarTransfer(receiver, amount).freezeWith(client);
...
async function contractBalanceCheckerFcn(contractId) {
	const contractQueryTx = new ContractCallQuery().setContractId(contractId).setGas(100000).setFunction("getBalance");
